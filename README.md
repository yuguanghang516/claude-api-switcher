...
- 使用深海蓝、瓷白和青蓝的 Interlink 弧带标识，统一应用到侧栏、窗口、任务栏和 EXE。
...
1. 从 GitHub Releases 下载并运行 `Claude API Switcher V4.6.1.exe`。
2. 双击运行。
...
V4.6.1 适配新版 gcli2api 的“抗截断/”模型别名，合并重复项，并在同档 Gemini 模型中优先排列更新的版本。Gemini 3.8 Flash 使用上游动态返回的真实模型 ID；具体可用性取决于凭证和配额。完整说明见 [`docs/releases/v4.6.1.md`](docs/releases/v4.6.1.md)。
