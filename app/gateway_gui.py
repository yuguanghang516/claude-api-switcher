...
class GatewayPanel:
    ...
    def _build_gateway_control(self):
        ...
        self.gw_url_label = ctk.CTkLabel(url_row, text="",
                                          text_color=INFO, font=ctk.CTkFont(family=FONT_MONO, size=14))
        self.gw_url_label.pack(side="left")

        self.copy_btn = ctk.CTkButton(url_row, text=t("copy_url", self.lang), width=70, height=36,
    ...
    def _build_model_section(self):
        ...
        self.search_entry = ctk.CTkEntry(
            head, textvariable=self.search_var, width=180, height=36,
        ...
        self.add_model_btn = self._bind_text(ctk.CTkButton(
            head, text="", width=110, height=36, fg_color=ACCENT,
    ...
    def _add_model_row(self, model: dict):
        ...
        ctk.CTkLabel(row, text=name_text, text_color=TEXT_PRIMARY,
                     font=ctk.CTkFont(family=FONT_FAMILY, size=14)).pack(side="left", padx=PAD_MD, pady=PAD_SM)
        ...
        ctk.CTkButton(actions, text=toggle_text, width=50, height=36,
                      fg_color=BG_ELEVATED, hover_color=BORDER, text_color=TEXT_PRIMARY,
                      font=ctk.CTkFont(family=FONT_FAMILY, size=14),
                      command=lambda m=model: self._toggle_model(m)).pack(side="left", padx=PAD_XS)
        ctk.CTkButton(actions, text=t("delete", self.lang), width=40, height=36,
    ...
    def _build_log_section(self):
        ...
        self.log_title = self._bind_text(ctk.CTkLabel(
            ...
            font=ctk.CTkFont(family=FONT_FAMILY, size=14, weight="bold")), "logs_tab")
        self.log_title.pack(side="left")

        self.refresh_log_btn = ctk.CTkButton(
            head, text=t("refresh_logs", self.lang), width=60, height=36,
            fg_color=BG_ELEVATED, hover_color=BORDER, text_color=TEXT_PRIMARY,
            font=ctk.CTkFont(family=FONT_FAMILY, size=14), command=self._refresh_logs)
        self.refresh_log_btn.pack(side="right", padx=PAD_XS)

        self.clear_log_btn = self._bind_text(ctk.CTkButton(
            head, text="", width=60, height=36, fg_color="transparent",
            hover_color=BORDER, font=ctk.CTkFont(family=FONT_FAMILY, size=14),
            command=self._clear_logs), "clear_logs")
        self.clear_log_btn.pack(side="right")

        self.log_text = ctk.CTkTextbox(frame, height=150, fg_color=BG_INPUT,
                                        font=ctk.CTkFont(family=FONT_MONO, size=14), state="disabled")
    ...
    def _refresh_gateway_status(self):
        ...
        if self.gateway.is_running():
            ...
            self.toggle_btn.configure(text=t("start_gateway", self.lang), fg_color=ACCENT)
